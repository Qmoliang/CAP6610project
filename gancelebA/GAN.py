...
class generator(nn.Module):
    ...
    def __init__(self, dataset = 'mnist'):
        ...
        self.fc = nn.Sequential(
            ...
            nn.BatchNorm1d(1024),
            nn.ReLU(),
            nn.Linear(1024, 128 * (self.input_height // 4) * (self.input_width // 4)), #128x7x7
        ...
        self.deconv = nn.Sequential(
            ...
            nn.ConvTranspose2d(128, 64, 4, 2, 1),   #(None,64,14,14)
            nn.BatchNorm2d(64),
            nn.ReLU(),
            nn.ConvTranspose2d(64, self.output_dim, 4, 2, 1), #(None,1,28,28)
            nn.Sigmoid(),
        )
    ...
    def forward(self, input):
        x = self.fc(input)
        x = x.view(-1, 128, (self.input_height // 4), (self.input_width // 4))  #(None,128,7,7)
        x = self.deconv(x)
...
class discriminator(nn.Module):
    ...
    def __init__(self, dataset = 'mnist'):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(self.input_dim, 64, 4, 2, 1),   #(None,64,14,14)
            nn.LeakyReLU(0.2),
            nn.Conv2d(64, 128, 4, 2, 1),  #   (None,128,7,7)
            nn.BatchNorm2d(128),
            nn.LeakyReLU(0.2),
        )
        self.fc = nn.Sequential(
...
class GAN(object):
    def __init__(self, args):
        ...
        if self.dataset == 'mnist':
            ...
            self.data_loader = utils.load_celebA('data', transform=transforms.Compose(
            ...
            for iter, (x_, _) in enumerate(self.data_loader):
                ...
                if ((iter + 1) % 100) == 0:
                    print("Epoch: [%2d] [%4d/%4d] D_loss: %.8f, G_loss: %.8f" %
                          ((epoch + 1), (iter + 1), self.data_loader.dataset.__len__() // self.batch_size, D_loss.data, G_loss.data))
